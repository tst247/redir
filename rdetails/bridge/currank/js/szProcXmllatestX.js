...
function szHeader() {ffOut='';/* <div class="szBlock'+szHeat+' headings">
	...
	ffOut='<div class="szBlock0 headings"></div>'+'<div id="r1">Free Briany</div>'+
	'<div id="r2">Rankings</div>'+ '<div id="r3"><em>'+szEventName+'</em></div>'+
		'<div id="hd"><div id="r4">Results in:'+gA['ResultsIn']+'</div>'+"\n";
	if (szEventNameSW==false) {szEventNameSW=true;gA['out'][szHeat]['EventName']=szEventName;console.log(szEventName);}
	gA['out'][szHeat]['ResultsIn'].push(gA['ResultsIn']);console.log(gA['ResultsIn']);
	// console.log(ffOut);
...
function szDetail(ff4Loop) {ffOut='',ffZeb='',ffB=false;console.log('szEWHeat',szEWHeat);
	...
	for (i=0;i<szSec[ff4Loop][szHeat].length; i++) {ffB=!ffB;ffZeb=ffB?'':' style="background:#333333"';
		...
		gA['out'][szHeat]['Rank'].push(szSec[ff4Loop][szHeat][i]['Rank']);
		gA['out'][szHeat]['PairName'].push(szSec[ff4Loop][szHeat][i]['PairName']);
		gA['out'][szHeat]['Score'].push(szSec[ff4Loop][szHeat][i]['Score']);		
		console.log('Rank',	szSec[ff4Loop][szHeat][i]['Rank'],'PairName',szSec[ff4Loop][szHeat][i]['PairName'],
		'Score',szSec[ff4Loop][szHeat][i]['Score']);	
